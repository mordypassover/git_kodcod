...
public class WeatherReadingsController : ControllerBase
{
    ...
    //  #2
    [HttpGet("{id}")]
    public ActionResult<IEnumerable<WeatherReading>> GetReadingById(int id)
    {
    ...
    public ActionResult<IEnumerable<WeatherReading>> GetReadingsByLocation(string location)
    {
        var readings = _readings.Where(r => r.Location
        .Equals(location, StringComparison.OrdinalIgnoreCase)).ToList();
    ...
    public ActionResult<IEnumerable<WeatherReading>> 
        SearchByTemperature([FromQuery] double? minTemp,
